...
$id = $_GET['id'];
$b = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM tb_portofolio WHERE id_portofolio='$id'"));
...
if (isset($_POST['input'])) {
    ...
    if (empty($foto)) {
        ...
        $qus = mysqli_query($koneksi, "UPDATE `tb_portofolio` SET `judul`='$judul',`text`='$isi' WHERE id_portofolio='$id'");
        if ($qus) {
            echo "<script>alert('input berhasil');document.location='../portofolio.php'</script>";
        } else {
        ...
        $sql = mysqli_query($koneksi, "UPDATE `tb_portofolio` SET `judul`='$judul',`text`='$isi',`gambar`='$newnama' WHERE id_portofolio='$id'");
        if ($sql) {
            echo "<script>alert('input berhasil');document.location='../portofolio.php'</script>";
        } else {
            echo "<script>alert('input gagal silahkan coba lagi');</script>";
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form action="" method="POST" autocomplete="off" enctype="multipart/form-data">
                                    ...
                                    <input type="text" class="form-control" name="judul" placeholder="Masukan Judul portofolio" value="<?= $b['judul']; ?>" size="50" required autocomplete="off">
                                    <br><br>
                                    <textarea name="isi" class="ckeditor" id="ckeditor" placeholder="Isi berita"><?= $b['text']; ?></textarea>
                                    <br>
